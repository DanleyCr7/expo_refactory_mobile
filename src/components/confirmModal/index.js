...
const ConfirmModal = ({ date }) => {
  ...
  const cancelMeal = () => {
    if (reason_for_cancellation != "") {
      api
        .put(`/reserves/cancel/${reserve?._id}`, {
          reason_for_cancellation: "teste",
        })
        .then(async (resp) => {
  ...
  return (
    <>
      <ContentQRCancel cancel={canRequired && required}>
        ...
        >
          <TextButton style={reserve?.approved ? {} : { color: Colors.GREEN }}>
            {reserve?.confirm == "sim"
              ? "Presença confirmada"
              : reserve?.cancel
              ? "Reserva cancelada"
      ...
      >
        <Box style={styles.boxContainer}>
          ...
          <TouchableOpacity style={styles.icon} onPress={onModalClose}>
            <Icon name="highlight-off" size={26} color={Colors.RED} />
          </TouchableOpacity>
        </Box>
      </Modal>
    </>
